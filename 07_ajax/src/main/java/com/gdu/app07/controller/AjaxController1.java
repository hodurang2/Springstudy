...
public class AjaxController1 {
  ...
  @RequestMapping(value="/list.do", method=RequestMethod.GET, produces="application/json; charset=UTF-8") // produces : 응딥 데이터 파일 
  public List<AjaxDto> list(){
    return ajaxService.getDtoList(); // jackson 라이브러리가 List<AjaxDto>를 json데이터로 자동 변환시켜준다.
  }
