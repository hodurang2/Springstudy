...
-- 3) 댓글 목록
-- 블로그 - 댓글 : 1:N (댓글이 달린 블로그 정보는 이미 상세보기에 모두 표시되어 있으므로 여기에선 조언할 필요가 없음)
-- 사용자 - 댓글 : 1:N (댓글이 사용자 이름을 표시)
SELECT A.COMMENT_NO, A.CONTENTS, A.BLOG_NO, A.CREATED_AT, A.STATUS, A.DEPTH, A.GROUP_NO, A.USER_NO, A.NAME
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY GROUP_NO DESC, DEPTH ASC, COMMENT_NO DESC) AS RN, COMMENT_NO, CONTENTS, BLOG_NO, CREATED_AT, STATUS, DEPTH, GROUP_NO, USER_NO, NAME
