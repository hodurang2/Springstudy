...
/*
 * @Mapper
 * 1. 매퍼의 쿼리문을 호출할 수 있는 인터페이스에 추가하는 마이바티스 어노테이션.
 * 2. 메소드이름과 호출할 쿼리문의 아이디(id)을 동일하게 맞추면 자동으로 호출된다.
 * 3. @Mapper가 등록된 인터페이스의 검색이 가능하도록 @MapperScan을 추가해야 한다. (SqlSessionTemplate Bean이 등록된 AppConfig.
 * 4. 매퍼에 namespace 값을 인터페이스 경로로 작성한다.
 */
@Mapper
